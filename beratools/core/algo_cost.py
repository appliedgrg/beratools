...
def alt_cost_raster(
    ...
    """
    Alternative version of cost_raster.
    ...
    def alt_cost_norm_dist_transform(canopy_ndarray, max_line_dist, sampling, tree_gaps=None):
        ...
        # Compute the Euclidean distance transform (edt) where the valid values are
        dist_transform_array = scipy.ndimage.distance_transform_edt(np.logical_not(np.isnan(in_ndarray)),
                                                                    sampling=sampling
                                                                    )
    ...
    def alt_dyn_np_cost_raster_refactor(canopy_ndarray, cc_mean, cc_std, cc_smooth, avoidance, cost_raster_exponent,
                                        tree_gaps=None):
        # Calculate the lower and upper bounds for canopy cover (mean ± std deviation)
        lower_bound = cc_mean - cc_std
        upper_bound = cc_mean + cc_std
